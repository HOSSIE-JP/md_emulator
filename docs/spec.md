...
- Editor 外部制御: MD Game Editor AI Control（localhost REST / MCP、opt-in 起動）

## Electron Plugin Runtime v2（開始段階）

Electron デスクトップ版の拡張は plugin runtime で統一する。

### MD Game Editor AI Control

AI Control はエミュレーターではなく Editor を操作するための外部制御面です。  
`AI Control` plugin から明示的に起動した場合のみ `127.0.0.1` に REST / MCP bridge を公開し、project state を変更する tool は `dryRun: true` または `confirm: true` を要求します。

REST と MCP は同じ tool registry を共有し、プロジェクト作成、設定更新、アセット登録、コード読み書き、プラグイン role 設定、generator 実行、ビルド、Test Play、エクスポートを orchestration 単位で提供します。

AI が実行した tool 呼び出しは操作ログとして保存し、REST / MCP の protocol、tool 名、引数と結果の要約、所要時間を Editor UI と `/v1/logs` から確認できるようにします。ログはライブ確認の入口であり、低レベルな UI クリック列ではなく意味のある editor 操作単位を観測対象にします。

### プラグイン種別

- `build`: Build ボタン時のコード生成・ビルドフック（単一選択）
- `editor`: サイドタブ上のゲーム固有編集機能
- `asset`: アセット管理機能（標準プラグイン化対象）
- `emulator`: Test Play ボタン時のエミュレーター起動フック
- `logger`: 汎用ログ出力先・整形機能
- `converter`: 画像変換等のモジュール機能（他プラグインから呼び出し）

### manifest v2（互換あり）

- `type` または `types` で種別を宣言
- `hooks` でサポートする hook 名を宣言
- 旧形式（`generateSource` のみ）も後方互換で受理

### hook 方針（初期）

- Build 系: `onBuildStart`, `onBuildLog`, `onBuildEnd`, `onBuildError`
- Emulator 系: `onTestPlay`
- Editor 系: `getTab`, `onActivate`, `onDeactivate`

### プロジェクト設定（project.json）

- `builderPlugin`: build plugin id
- `emulatorPlugin`: emulator plugin id
- `loggerPlugins`: logger plugin id 配列
