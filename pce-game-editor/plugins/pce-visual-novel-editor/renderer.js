...
function defaultDoc(assets = []) {
  ...
  return {
    ...
    scenes: [{
      ...
      characters: first('sprite') ? [{ assetId: first('sprite'), x: 176, y: 72, pose: 'default' }] : [],
      messages: [
        { speaker: 'アカリ', text: 'こんにちは、PCエンジンの世界へ。', voiceAssetId: first('adpcm'), advanceMode: 'button' },
        { speaker: 'アカリ', text: 'ボタンでメッセージを送ります。', voiceAssetId: '', advanceMode: 'button' },
      ],
...
export function activatePlugin({ root, api, registerCapability }) {
        ...
        <section class="pce-vn-edit">
          ...
          <form class="pce-vn-form" data-role="scene-form">
            ...
            <div class="pce-vn-grid tight">
              <label class="form-group"><span class="form-label">立ち絵 X</span><input class="form-input" name="characterX" type="number" min="0" max="255" /></label>
              <label class="form-group"><span class="form-label">立ち絵 Y</span><input class="form-input" name="characterY" type="number" min="0" max="255" /></label>
            </div>
          </form>
  ...
  function selectedSceneFromForm() {
    ...
    return {
      ...
      characters: characterAssetId ? [{
        ...
        x: Math.max(0, Math.min(255, asNumber(form.elements.characterX.value, 176))),
        y: Math.max(0, Math.min(255, asNumber(form.elements.characterY.value, 72))),
        pose: 'default',
      }] : [],
  ...
  async function renderPreview() {
    ...
    const characterDef = current.characters[0] || { x: 176, y: 72 };
    characterPreview.style.left = `${(characterDef.x / 256) * 100}%`;
    characterPreview.style.top = `${(characterDef.y / 224) * 100}%`;
  ...
  function renderForm() {
    ...
    form.elements.characterX.value = character.x ?? 176;
    form.elements.characterY.value = character.y ?? 72;
