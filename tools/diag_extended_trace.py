"""Extended frame trace - check when sound starts"""
import requests
...
for frame in range(0, 2001, 50):
    ...
    print(f"F{frame:5d}: PC=0x{pc:06X} IPL={ipl} Z80=0x{z80_pc:04X} "
          f"z80cmd=0x{cmd:02X} YM_delta={ym_delta:6d} "
...
print(f"YM histogram port0: {hist[:30]}")
hist1 = apu.get("ym_histogram_port1_nonzero", [])
print(f"YM histogram port1: {hist1[:30]}")
