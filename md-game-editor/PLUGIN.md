...
| renderer capability | `midi-convert-ui` |

MIDI ファイルを plugin-local の JavaScript converter で Mega Drive 向け VGM に変換し、必要に応じて SGDK `xgmtool.exe` で XGM も生成します。VGM が生成済みなら XGM 変換失敗は warning として返し、`XGM2 <symbol> music/<symbol>.vgm` 形式のアセット登録 payload を提供します。Python ランタイムには依存しません。
