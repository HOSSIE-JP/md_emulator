 ...
 *
 * ボタン → レーン対応表:
 *   BUTTON_UP    → NOTE_UP    (レーン0)
 *   BUTTON_DOWN  → NOTE_DOWN  (レーン1)
 *   BUTTON_LEFT  → NOTE_LEFT  (レーン2)
 ...
 *
 * 複数ボタンが同時にセットされている場合、
 * 優先度の高い方（UP→DOWN→LEFT→RIGHT→A→B→C）が返る。
 *
 * @param button SGDKのボタンビットマスク (BUTTON_UP, BUTTON_A 等)
 * @return 対応レーン番号 (NOTE_UP..NOTE_C)、該当なしなら -1
 */
 ...
 *
 * @return レーン状態のビットフラグ (bit0=NOTE_UP .. bit6=NOTE_C)
 */
