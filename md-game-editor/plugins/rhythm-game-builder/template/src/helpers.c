...
void fadeInBlue(u16 fromcol, u16 tocol, const u16* pal, u16 numframeBlue, u16 numframeGreenRed) {
	...
	for (u16 i = 0; i < (tocol - fromcol) + 1; i++) palBlue[i] = pal[i] & 0xF00;
...
void resetSpriteHelper(void) {
	vdpSpriteIndexNext = 0;
	vdpSpriteTileIndexNext = 96;
}
...
void addSprite(s16 x, s16 y, u16 attribute) {
	...
	VDP_setSpriteFull(vdpSpriteIndexNext, 320, 0, SPRITE_SIZE(4, 4), attribute, 0);

	// Link to previous sprite
	if (vdpSpriteIndexNext > 0) VDP_setSpriteLink(vdpSpriteIndexNext - 1, vdpSpriteIndexNext);
...
void moveSprites(s16 x, s16 y) {
	...
	for (u16 i = 0; i < vdpSpriteIndexNext; i++)
		VDP_setSpritePosition(
			i, min(320, max(-32, spriteOffsets[i].x + x)), min(224, max(-32, spriteOffsets[i].y + y)));
...
void initMenu(MenuItem* items, u16 num) { initMenuEx(items, num, "\x7F"); }
...
void updateMenuItems() {
	...
	if ((menuJoyStateOld != 0) && (menuButtonDelay > 0)) {
		...
		if (joyState & BUTTON_A) {
			if (item->type == MIT_BOOL) *item->vbool.value = TRUE;
		} else if (joyState & BUTTON_LEFT) {
			if (item->type == MIT_S16) *item->vs16.value = max(*item->vs16.value - item->vs16.step, item->vs16.min);
		} else if (joyState & BUTTON_RIGHT) {
			if (item->type == MIT_S16) *item->vs16.value = min(*item->vs16.value + item->vs16.step, item->vs16.max);
		} else if (joyState & BUTTON_UP) {
			if (menuLine > 0) menuLine--;
		} else if (joyState & BUTTON_DOWN) {
			if (menuLine < menuItemsNum - 1) menuLine++;
		} else {
			if (item->type == MIT_BOOL) *item->vbool.value = FALSE;
		}
